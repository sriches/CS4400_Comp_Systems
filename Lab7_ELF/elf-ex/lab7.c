...
int main(int argc, char **argv) 
{
  ...
  /* PART 1 - Print the offset to the section table and offset to 
  index within the section that contains section-name strings */
  Elf64_Shdr *shdrs = (void*)ehdr+ehdr->e_shoff;
  printf("%X\n", shdrs);   // Address to section header
  printf("%X\n", ehdr->e_shstrndx); // Address to strings table index 
