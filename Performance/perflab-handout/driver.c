...
static int check_pinwheel(int dim, int save_images)
{
    ...
    for (qi = 0; qi < 2; qi++)
      for (qj = 0; qj < 2; qj++)
        for (i = 0; i < dim/2; i++)
          for (j = 0; j < dim/2; j++) {
            int s_idx = RIDX((qj * dim/2) + i, j + (qi * dim/2), dim);
            int d_idx = RIDX((qj * dim/2) + dim/2 - 1 - j, i + (qi * dim/2), dim);
            tmp[d_idx].red = (orig[s_idx].red + orig[s_idx].green + orig[s_idx].blue) / 3;
            tmp[d_idx].green = (orig[s_idx].red + orig[s_idx].green + orig[s_idx].blue) / 3;
            tmp[d_idx].blue = (orig[s_idx].red + orig[s_idx].green + orig[s_idx].blue) / 3;
          }
...
{
  orig[-1].dim = dim;
  benchmarks_motion[idx].motion_funct(orig, result);
}
